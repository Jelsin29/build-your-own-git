...
int cmd_init(int argc, char *argv[])
{
    ...
    case 1:
        fprintf(stdout,
                "Reinitialized existing mygit repository in %s/.mygit/\n",
                path);
        return EXIT_SUCCESS;
    default:
...
static const char *detect_object_type(const unsigned char *store,
                                      size_t store_len)
{
...
int cmd_commit_tree(int argc, char *argv[])
{
    ...
    if (tree_hex == NULL || message == NULL) {
        fprintf(stderr,
                "usage: mygit commit-tree <tree-sha> [-p <parent>] "
                "-m <message>\n");
        return EXIT_FAILURE;
    }
