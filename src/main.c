...
/*
 * Diff a single file: read content from old_hex blob and new text,
 * compute Myers diff, print unified output.
 */
...
static int cmd_diff(int argc, char *argv[])
{
    ...
    if (cached) {
        ...
        ret = mygit_head_resolve(".", head_hex);
        if (ret == 0) {
            mygit_commit c;
            if (mygit_commit_read(".", head_hex, &c) == 0) {
                flatten_tree(".", c.tree_hex, "", &head_files,
        ...
        /* For each index entry, diff against HEAD version */
        for (size_t i = 0; i < idx.count; i++) {
            char idx_hex[41];
            mygit_hex(idx.entries[i].sha1, idx_hex);

            /* Find in HEAD */
            const char *old_hex = NULL;
            for (size_t j = 0; j < head_count; j++) {
                if (strcmp(idx.entries[i].name, head_files[j].name) == 0) {
                    if (memcmp(idx.entries[i].sha1, head_files[j].sha1, 20) != 0) {
                        char tmp[41];
                        mygit_hex(head_files[j].sha1, tmp);
                        old_hex = tmp;

                        /* Read new (index) blob content */
            ...
            if (old_hex == NULL) {
                ...
                if (!found) {
                    ...
                    if (mygit_blob_read(".", idx_hex, &new_data,
                                        &new_len) == 0) {
                        diff_blobs(idx.entries[i].name, NULL,
                                   new_data, new_len);
                        free(new_data);
                    }
                }
        ...
        /* Default: compare working tree vs index */
        for (size_t i = 0; i < idx.count; i++) {
            char idx_hex[41];
            mygit_hex(idx.entries[i].sha1, idx_hex);
            ...
            if (fp == NULL) continue; /* deleted in working tree — skip */
            ...
            /* Hash working tree content to check if it differs */
            char wt_hex[41];
